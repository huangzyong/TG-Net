...
def Unet_G(init_type='kaiming', init_gain=0.01, gpu_id='cuda:0'):
    net = Unet(in_channels=3, n_cls=2, n_filters=16)
    return init_net(net, init_type, init_gain, gpu_id)


def ResUnet_G(init_type='kaiming', init_gain=0.01, gpu_id='cuda:0'):
    net = ResUnet(in_channels=3, n_cls=2, n_filters=16)
    return init_net(net, init_type, init_gain, gpu_id)


def AttUnet_G(init_type='kaiming', init_gain=0.01, gpu_id='cuda:0'):
    net = AttUnet(in_channels=3, n_cls=2, n_filters=16)
    return init_net(net, init_type, init_gain, gpu_id)
...
def Li_Dense_Unet_G(init_type='kaiming', init_gain=0.01, gpu_id='cuda:0'):
    net = Dense_unet(in_channels=2, n_cls=2, n_filters=16)
    return init_net(net, init_type, init_gain, gpu_id)
...
class ResUnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.block_4_2_left(self.block_4_1_left(self.pool_3(ds2))) + self.res_4(self.pool_3(ds2))
...
class Zhao(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.n_cls == 1:
            return torch.sigmoid(x + x_1 + x_2 + x_3)
        else:
            return F.softmax(x, dim=1)
